...
\begin{abstract}
The majority of historical epidemiological data remains undigitized. This paper evaluates the effectiveness of automated table extraction as an alternative to manual data entry. We assess the performance of \emph{Amazon Textract} on a large dataset of historical infectious disease records, comparing extraction accuracy, error characteristics, and overall efficiency against traditional manual transcription methods. Cell-level agreement was evaluated on the 1956 tables (53{,}872 evaluated cells), where content-only agreement between the automated and manually transcribed data was 88.5\% overall and 95.6\% among cells that were numeric on both sides; downstream epidemiological comparisons (disease peaks, growth rates, and cross-correlations) span all three years of the dataset, 1956--1958. Our findings show that extraction errors were present in all extractions we performed; they tend to be either minor with negligible impact or discernible on inspection. Extraction runtime was short relative to an extrapolated manual-entry estimate, but total person-time, including post-processing, was not measured. As well, the measured post-processing time we did record was a small proportion of the total time previously required for the whole task. Overall, this paper illustrates the potential and limitations of a human-in-the-loop workflow for transforming historical tabular data of this kind into digital formats.
\end{abstract}

\section*{Author Summary }
Vast amounts of historical public health data exist only as hardcopy or scanned images, making them inaccessible for statistical analysis. We tested whether \emph{Amazon Textract}, a machine learning service built for structured document extraction, could bridge this gap using historical Canadian infectious disease records. After modest post-processing, the automated data closely matched records entered by hand: content-only agreement between the two sources was 88.5\% across all evaluated cells, rising to 95.6\% for cells holding numeric case counts. Most errors that did occur differed from the reference value by only one or two characters and had negligible statistical impact; a small number of errors were large and occurred specifically in disease-name strings. Key downstream epidemiological statistics, including disease peak timing, initial growth rates, and cross-correlations between diseases, showed only small, quantified differences whether calculated from the automated or manually entered data. For researchers working with historical archives, this case study suggests that tools like Textract may reduce the extraction-time component of digitizing data, though total person-time including post-processing was not measured here. We hope this work supports careful benchmarking and auditing of current extraction tools on representative source material in historical epidemiology and public health research.
...
Of course, Levenshtein distances of large magnitudes help us in the sense that, following a manual inspection of the extracted data, it would be clear to a human that an error has occurred. In addition, distances of this magnitude occurred exclusively in disease-name strings rather than in numeric values.
...
\begin{figure}[h!]
    ...
    \caption{Comparison of manual versus Textract disease cross-correlation for Chickenpox and Measles. This is described in \cref{sec:measuresAccuracy}. Data version: processed Textract data (\cref{tab:pipeline}, Stage 2), without manual correction.}
    \label{fig:comparisonCrossCorrelation}
\end{figure}

To gain a sense of how similar the cross-correlations are, we computed a set of summary statistics across all lags (\cref{tab:ccf_summary}; per-lag values are in \nameref{sec:supp}, Table S1). We report absolute and root-mean-square differences rather than a relative difference at each lag, because a relative difference is uninformative wherever the manual correlation is itself near zero, as the next comparison illustrates.

Both datasets peak at lag 0 (\cref{tab:ccf_summary}), so the change in peak lag is 0 and the change in peak correlation is small ($-0.0271$). Across all 25 lags, the largest absolute discrepancy between the two series is $0.0832$ (at lag 7) and the root-mean-square difference is $0.0526$. In this disease-pair comparison, the lag with the highest cross-correlation was unchanged after processing. Because cross-correlation is a measure of time relationships rather than exact magnitudes, the estimated timing structure of this pairwise comparison was similar between the two sources.

We repeat this process with the time series for Measles and Meningitis (incidence curves in \cref{fig:comparisonIncidentsMM}, cross-correlation in \cref{fig:comparisonCrossCorrelation2}), and compare the values using the same summary statistics (\cref{tab:ccf_summary}; per-lag values are in \nameref{sec:supp}, Table S2). Unlike the Measles/Chickenpox comparison above, the Meningitis series used here has been manually corrected: additional post-processing was applied to remove visible outliers, as described in \cref{sec:hardcoding}.
...
The cross-correlation between Measles and Meningitis is weak across all lags in both the manual and Textract datasets, with values generally between about $-0.11$ and $0.09$ (\cref{tab:ccf_summary}). Because these correlations are close to zero throughout, this pair is the one for which a relative-difference metric was least informative: at several lags the manual correlation itself is close to zero, so any ratio against it is unstable regardless of how small the absolute discrepancy is. The manual series peaks at lag $-9$ and the Textract series at lag $-11$, a change in peak lag of $-2$ weeks; the change in peak correlation is small ($-0.0059$). The largest absolute discrepancy between the two series is $0.1108$ (at lag $-7$, where Textract crosses to a small negative value while the manual value remains small and positive) and the root-mean-square difference is $0.0423$ -- both smaller in absolute terms than the Measles/Chickenpox discrepancies above, even though a relative-difference expression of this same gap would read an order of magnitude larger, for the reason given above. Textract captures the overall cross-correlation pattern here, and the largest discrepancies occur at lags where both correlations are already small.
...
\begin{figure}[h!]
    ...
    \caption{Comparison of manual versus Textract disease cross-correlation for Measles and Meningitis. Data version: manually corrected (outlier removal, \cref{sec:hardcoding}).}
...
We see in \cref{fig:comparisonCrossCorrelation3} that the graphs of the cross-correlation calculated between Chickenpox and Mumps are very similar between the manual and Textract data. The summary statistics in \cref{tab:ccf_summary} (per-lag values in \nameref{sec:supp}, Table S3) confirm this: the differences between the two datasets at each lag are small.
...
\begin{figure}[h!]
    ...
    \caption{Comparison of manual versus Textract disease cross-correlation for Chickenpox and Mumps. Data version: manually corrected (\cref{sec:hardcoding}).}
    \label{fig:comparisonCrossCorrelation3}
\end{figure}

The cross-correlation between diseases and the conclusions we draw from their CCF data are almost identical in both the manual and Textract dataset for this pair: the peak lag, which is most important for epidemiological study, is identical (lag 0 in both), the change in peak correlation is negligible ($-0.0016$), and both the largest absolute discrepancy ($0.0205$, at lag 12) and the root-mean-square difference ($0.0106$) are the smallest of the three disease pairs examined.
...
\begin{table}[h]
\centering
\begin{tabular}{lp{2.7cm}ccccccccc}
\toprule
Disease pair & Data version & Max abs diff & Lag at max & RMS diff & Peak lag M & Peak lag T & $\Delta$ peak lag & Peak $\rho$ M & Peak $\rho$ T & $\Delta$ peak $\rho$ \\
\midrule
Measles vs Chickenpox & processed, no manual correction & 0.0832 & 7 & 0.0526 & 0 & 0 & 0 & 0.8600 & 0.8329 & $-0.0271$ \\
Measles vs Meningitis & \textbf{manually corrected} & 0.1108 & $-7$ & 0.0423 & $-9$ & $-11$ & $-2$ & 0.0862 & 0.0803 & $-0.0059$ \\
Chickenpox vs Mumps & \textbf{manually corrected} & 0.0205 & 12 & 0.0106 & 0 & 0 & 0 & 0.8990 & 0.8974 & $-0.0016$ \\
\bottomrule
\end{tabular}
\caption{Summary of cross-correlation agreement between manual and Textract series for all three disease pairs, computed by \texttt{analysis/ccf\_summary.py} from the per-lag CCF values (\nameref{sec:supp}, Tables S1--S3). Lag range $k \in \{-12,\dots,+12\}$ (n = 25 lags). Sign convention: all differences are Textract $-$ Manual. ``M''/``T'' denote Manual/Textract. Peak-lag ties (S3.2: smaller $|k|$, then negative lag) did not occur for any of the three pairs. \textbf{Data version} follows \cref{tab:pipeline}: two of the three pairs (Meningitis, Mumps) use Textract series manually corrected following inspection (\cref{sec:hardcoding}); Measles vs.\ Chickenpox does not.}
...
\end{enumerate}
If too much human correction is necessary, the time saved becomes relatively small, so the tool is not successful. As well, even with modest time saving, data correction is a less pleasant task for humans than data entry. In our own preliminary exploration (\nameref{sec:supp}, S5 Text), the conversational model we tried before Textract produced errors prevalent and tedious enough to fix that we abandoned it in favour of a tool purpose-built for table extraction.
...
We would like to express sincere gratitude to Dr. Ben Bolker for his valuable direction and Noah Juravsky for the many insightful discussions and technical support that significantly accelerated the progress of this work. 
\clearpage
\printbibliography 
...
\midrule
-12 & 0.0710 & 0.0244 & -0.0467 \\
-11 & 0.1472 & 0.0871 & -0.0601 \\
-10 & 0.2187 & 0.1571 & -0.0616 \\
 -9 & 0.2841 & 0.2199 & -0.0642 \\
 -8 & 0.3621 & 0.3213 & -0.0408 \\
 -7 & 0.4439 & 0.3996 & -0.0443 \\
 -6 & 0.5214 & 0.4891 & -0.0323 \\
 -5 & 0.5862 & 0.5315 & -0.0548 \\
 -4 & 0.6308 & 0.5877 & -0.0431 \\
 -3 & 0.6781 & 0.6264 & -0.0518 \\
 -2 & 0.7497 & 0.7009 & -0.0488 \\
 -1 & 0.8128 & 0.7766 & -0.0362 \\
  0 & 0.8600 & 0.8329 & -0.0271 \\
  1 & 0.8380 & 0.8078 & -0.0301 \\
  2 & 0.8146 & 0.7682 & -0.0464 \\
  3 & 0.7926 & 0.7385 & -0.0542 \\
  4 & 0.7705 & 0.7412 & -0.0292 \\
  5 & 0.7337 & 0.6668 & -0.0669 \\
  6 & 0.6788 & 0.6102 & -0.0686 \\
  7 & 0.6369 & 0.5536 & -0.0832 \\
  8 & 0.5737 & 0.5395 & -0.0341 \\
  9 & 0.5136 & 0.4662 & -0.0474 \\
 10 & 0.4622 & 0.3991 & -0.0631 \\
 11 & 0.4155 & 0.3386 & -0.0769 \\
 12 & 0.3608 & 0.3091 & -0.0516 \\
\bottomrule
\end{tabular}
\end{table}
...
Lag & Manual & Textract & Difference \\
\midrule
-12 & 0.0630 & 0.0620 & -0.0010 \\
-11 & 0.0472 & 0.0803 & 0.0331 \\
-10 & 0.0495 & 0.0648 & 0.0153 \\
 -9 & 0.0862 & 0.0593 & -0.0269 \\
 -8 & 0.0706 & 0.0086 & -0.0620 \\
 -7 & 0.0707 & -0.0402 & -0.1108 \\
 -6 & 0.0175 & -0.0603 & -0.0778 \\
 -5 & -0.0096 & -0.0299 & -0.0203 \\
 -4 & 0.0108 & -0.0341 & -0.0449 \\
 -3 & -0.0092 & -0.0395 & -0.0303 \\
 -2 & 0.0195 & 0.0041 & -0.0153 \\
 -1 & 0.0640 & 0.0406 & -0.0235 \\
  0 & 0.0631 & 0.0315 & -0.0316 \\
  1 & 0.0400 & 0.0223 & -0.0178 \\
  2 & 0.0677 & 0.0532 & -0.0145 \\
  3 & 0.0796 & 0.0538 & -0.0258 \\
  4 & 0.0549 & 0.0508 & -0.0041 \\
  5 & 0.0023 & -0.0034 & -0.0057 \\
  6 & 0.0167 & 0.0152 & -0.0015 \\
  7 & 0.0155 & 0.0275 & 0.0120 \\
  8 & -0.0116 & -0.0240 & -0.0125 \\
  9 & -0.0792 & 0.0018 & 0.0810 \\
 10 & -0.0684 & 0.0055 & 0.0740 \\
 11 & -0.0482 & -0.0357 & 0.0125 \\
 12 & -0.1086 & -0.0678 & 0.0408 \\
\bottomrule
\end{tabular}
\end{table}
...
Lag & Manual & Textract & Difference \\
\midrule
-12 & 0.2990 & 0.2919 & -0.0072 \\
-11 & 0.3700 & 0.3638 & -0.0062 \\
-10 & 0.4427 & 0.4373 & -0.0054 \\
 -9 & 0.5025 & 0.4994 & -0.0032 \\
 -8 & 0.5564 & 0.5535 & -0.0029 \\
 -7 & 0.6161 & 0.6112 & -0.0049 \\
 -6 & 0.6742 & 0.6710 & -0.0032 \\
 -5 & 0.7127 & 0.7082 & -0.0045 \\
 -4 & 0.7570 & 0.7543 & -0.0028 \\
 -3 & 0.7924 & 0.7857 & -0.0067 \\
 -2 & 0.8258 & 0.8146 & -0.0113 \\
 -1 & 0.8714 & 0.8703 & -0.0011 \\
  0 & 0.8990 & 0.8974 & -0.0016 \\
  1 & 0.8505 & 0.8448 & -0.0057 \\
  2 & 0.7992 & 0.7878 & -0.0114 \\
  3 & 0.7582 & 0.7448 & -0.0134 \\
  4 & 0.7265 & 0.7115 & -0.0149 \\
  5 & 0.6898 & 0.6734 & -0.0164 \\
  6 & 0.6325 & 0.6177 & -0.0148 \\
  7 & 0.5645 & 0.5511 & -0.0134 \\
  8 & 0.4762 & 0.4643 & -0.0119 \\
  9 & 0.3993 & 0.3896 & -0.0097 \\
 10 & 0.3410 & 0.3265 & -0.0145 \\
 11 & 0.2643 & 0.2457 & -0.0186 \\
 12 & 0.1882 & 0.1677 & -0.0205 \\
\bottomrule
\end{tabular}
\end{table}
...
The Canadian Disease Incidence Dataset (CANDID) used in this study is publicly available through McMaster University's Theoretical Biology group \cite{CANDID}. All Python scripts used for data extraction and post-processing are available as supporting information accompanying this manuscript. Data underlying all figures and tables reported in this manuscript are available in the supporting information files.
