...
# Run through all files in input folder
# For each file, run the cleaning process
# Save cleaned file to a different folder with the input folder name and "_cleaned" suffix
input_folder <- "Compare\\cdi_ca_1956_wk_prov_dbs_textract"
output_folder <- sprintf("%s_cleaned", basename(input_folder))
dir.create(output_folder, showWarnings = FALSE)
for (input_file in list.files(input_folder, full.names = TRUE)) {
  cat("Processing file:", input_file, "\n")

  # For each file in the input folder, set output file path to the output folder
